...
ti.init(arch=ti.gpu)  # Use 80% of GPU memory
...
ex = ti.field(dtype=ti.f64, shape=9)
ey = ti.field(dtype=ti.f64, shape=9)
w = ti.field(dtype=ti.f64, shape=9)

rho = ti.field(dtype=ti.f64, shape=(ny, nx))
tau = ti.field(dtype=ti.f64, shape=(ny, nx))
u = ti.field(dtype=ti.f64, shape=(2, ny, nx))
Fg = ti.field(dtype=ti.f64, shape=(2, ny, nx))
nodetype = ti.field(dtype=ti.i32, shape=(ny, nx))
f = ti.field(dtype=ti.f64, shape=(ny, nx, 9))
f_old = ti.field(dtype=ti.f64, shape=(ny, nx, 9))
        ...
        if nodetype[i, j] <= 0:
            for q in range(1, 9):  
                ...
                if nodetype[per_i, per_j] <= 0:
                    f[i, j, q] = f_old[per_i, per_j, q]
                else:  
                    bounce_q = q + 4 if q < 5 else q - 4
                    f[i, j, q] = f_old[i, j, bounce_q]
...
def test_lb():
    ...
    plt.savefig("profile_taichi.png", dpi=300)
