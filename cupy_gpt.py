...
def collision(f, rho, ux, uy, obstacles, ex, ey, es, w, omega=1):
    ...
    f *= (obstacles <= 0)  # Mask solid nodes
    return f
...
def propogation(f, ex, ey, obstacles):
    ex = ex.astype(int)
    ey = ey.astype(int)    
    solid_node_in_path = cp.zeros(f.shape)
    fc_bb = cp.zeros(f.shape)
    solid_node_exist = (obstacles > 0).astype(cp.float32)
    idx = cp.array([0, 3, 4, 1, 2, 7, 8, 5, 6])        
...
def test_lb():
    ...
    rowlength = ly
    collength = lx
    rho = cp.zeros((rowlength, collength))
    ux = cp.zeros((rowlength, collength))
    uy = cp.zeros((rowlength, collength))
    obstacles = cp.zeros((rowlength, collength))
    obstacles[0, :] = 1
    obstacles[-1, :] = 1

    f = cp.zeros((9, rowlength, collength))
    f[1:5, :, :] = 1./9.
    f[5:9, :, :] = 1./36.
    f[0, :, :] = 4./9. 
